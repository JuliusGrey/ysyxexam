...
extern char end;
 __uint64_t pbreak = (__uint64_t)&end;
void *_sbrk(intptr_t increment) {
  __uint64_t old_pbreak = pbreak;
  __uint64_t new_pbreak = pbreak + increment;
  _syscall_(SYS_brk, (__uint64_t)&pbreak, new_pbreak, 0);
...
int _execve(const char *fname, char * const argv[], char *const envp[]) {
  // _exit(SYS_execve);
  // return 0;
  return _syscall_(SYS_execve, (__uint64_t)fname, 0, 0);
}
