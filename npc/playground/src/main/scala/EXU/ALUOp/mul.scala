...
class mul(val muxW : Int) extends Module{
  val io = IO(new Bundle() {
    ...
    val result_hi = Output(UInt(muxW.W))
    val result_low = Output(UInt(muxW.W))
  ...
  val idleMux = Mux(io.mul_valid ,mulState, idleState)
...
//    )
  ...
  io.result_low := resReg(muxW - 1,0).asUInt()
  io.result_hi := resReg(muxW*2 -1,muxW).asUInt()
...
object mulgen extends App {
  chisel3.Driver.execute(args,() => new mul(3) )
